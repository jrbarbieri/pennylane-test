...
export default function RecipesPage() {
  ...
  const fetchRecipes = (newQuery = query, newPage = 1, fromSearch = false, fromQuickMenu = false) => {
    fetchRecipesService({ query: newQuery, page: newPage })
      .then(data => {
        setQuery(newQuery)
        setPage(newPage)
  ...
  return (
    <>
      <Header page={page} totalPages={totalPages} onPageChange={p => fetchRecipes(query, p)} />
      <div style={{ display: 'flex', margin: '2rem' }}>
        <div style={{ width: '250px', marginRight: '2rem', display: 'flex', flexDirection: 'column' }}>
          <SearchSidebar
            query={query || ''}
            onSearch={q => {
              setActiveCategory('')
        ...
        <div style={{ flex: 1 }}>
          {recipes.length === 0 ? <EmptyState /> : <RecipeCards recipes={recipes} />}
        </div>
      </div>
    </>
  )
